...
public abstract class AbstractGoTween
{
	...
	public GoUpdateType updateType { get; set; }
	public GoLoopType loopType { get; set; }
	...
	public bool isCompleted { get { return _didComplete; }  } // have been completed ?
	...
	public float delay // delay before starting the actual animations
	{
		get { return _delay; }
		set { _delay = value; _delayComplete = ( _delay <= 0.0f ); }
	}
	internal float _elapsedDelay; // total time delayed
	protected bool _delayComplete = true; // once we complete the delay this gets set so we can reverse and play properly for the future
	
	// action event handlers
	protected Action<AbstractGoTween> _onStart; // executes only once at initial startup
	protected Action<AbstractGoTween> _onComplete; // executes whenever a Tween completes
	protected Action<AbstractGoTween> _onIterate; // executes whenever a Tween iterates (if looping)


	public override string ToString ()
	{
		return "[" + id + "] " + _totalElapsedTime + "/" + totalDuration;
	}


	public AbstractGoTween setOnStartHandler ( Action<AbstractGoTween> onStart )
	{
		_onStart = onStart;
		return this;
	}
	...
	public AbstractGoTween setOnCompleteHandler ( Action<AbstractGoTween> onComplete )
	{
	...
	public AbstractGoTween setOnIterateHandler ( Action<AbstractGoTween> onIterate )
	{
	...
	public virtual bool update( float deltaTime )
	{
		...
		{
			// Debug.Log ( "consuming delay " + _elapsedDelay + "/" + delay + " with delta " + deltaTime + " on " + id );
		...
		int priorCompletedIterations = _completedIterations;
		...
		{
			// we've ended an iteration
			if ( priorCompletedIterations != _completedIterations )
				onIterate ();
	...
	/// <summary>
	/// finish current tween then stop looping.
	/// </summary>
	public void stopLooping ()
	{
		loopCount = _completedIterations + 1;
	}
...
// 		update ( time - _elapsedTime );
		// for that we need to integrate delay in duration, totalDuration, etc.
		// we could make totalDuration a property auto computed
		// and check each usage of duration & totalDuration
	}
