...
public class GoTween : AbstractGoTween
{
	...
	public override bool update( float deltaTime )
	{
		...
		if( Go.validateTargetObjectsEachTick )
		{
			...
			if( target == null || target.Equals(null) )
			{
				...
				Debug.LogWarning ( "target validation failed. destroying the tween to avoid errors. Target name: " + this.targetName );
				autoRemoveOnComplete = true;
				return true;
			}
		}
	...
	// override to prepare all the TweenProperties for use
	protected override void onStart()
	{
		base.onStart();
		
		// prepare our TweenProperties
		if ( !_areTweenPropertiesPrepared )
		{
			_areTweenPropertiesPrepared = true;
			for ( var i = 0; i < _tweenPropertyList.Count; i++ )
...
// 	}
	...
	public void rewind( bool skipDelay )
	{
		...
		_delayComplete = skipDelay || ( delay == 0 );
